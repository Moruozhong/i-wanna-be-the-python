...
Python + [pygame-ce](https://pygame-ce.github.io/) 实现的 **I Wanna 类高精度平台跳跃游戏引擎**（含可视化地图编辑器）。

帧级物理（固定 50 FPS，`px/frame`）、精确 11×21 碰撞箱、无惯性离散移动，目标是接近 GameMaker 版 I Wanna 的手感与结构。

## 特性

- 🎮 **帧级物理**：固定 50 FPS、速度单位 `px/frame`、11×21 碰撞箱、离散 AABB 碰撞、像素级尖刺/子弹碰撞遮罩（由贴图 alpha 逐像素生成）
- 💀 **死亡演出**：红色粒子迸发 + 头高速弹出弹跳 + 彩虹拖尾 + 变色拉伸 + 渐显变黑，按 `R` 复活
- 🧗 **藤蔓（单向通道）**：非梯子非固体，只有从攀爬面一侧能吸附，另一侧自由穿过；吸附/跳出/滑落/二段跳语义完整
- 💧 **三种水**：水中跳跃不消耗次数、出水补记一段、二段水接触刷新、零段水禁跳
- ⭐ **跳跃星星 / 跳跃球**：改变最多跳跃次数（1/2/3 段）、返还一次跳跃
- 🗺️ **JSON 关卡**：`rooms/*.json`，出口切房、跨房间存档点、Checkpoint 持久化存档
- 🎨 **可视化地图编辑器**：三栏布局、图层/撤销/网格（32/16/8px 像素级放置）/自定义材质/背景/音乐/路径节点移动系统/一键测试
- 🚚 **路径节点**：与节点重合的任何元素沿轨迹往复移动，可设速度与触发方式，死亡回原位
...
- **水**：一段(浅蓝)/二段(蓝)/零段(浅灰)，统一压低下落速度；**水中跳不消耗次数**；一段/二段水离水（空中）补记一段、只剩一次跳；二段水进入/落地刷新（泳池无限跳）；零段水禁跳保留次数
- **藤蔓**：`tengwan_right` 攀爬面在右缘、`tengwan_left` 在左缘，只有从该侧撞上才吸附；`Shift+反方向` 跳出、反方向普通脱离、无输入/靠近下滑、`Shift+上` 死键；上攀靠"跳出再跳回"
- **单向平台**：32×16，只能从上方站立，向上跳跃不被阻挡
- **跳跃球**：返还一次跳跃，一次性，死亡恢复
- **跳跃星星**：一段黑/二段灰/三段黄，改最多跳跃次数，不可消耗，切房保留
- **Checkpoint 存档**：`save/save.json` 持久化，删除即重置
- **背景音乐**：房间自定义 BGM，死亡淡出、复活从淡出位置续播
